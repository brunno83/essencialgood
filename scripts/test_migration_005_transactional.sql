-- ============================================================================
-- SCRIPT DE TESTE TRANSACTIONAL NÃO-DESTRUTIVO DA MIGRATION 005
-- Executa a migration inteira, valida estruturas, permissões, RLS e RPC
-- com asserções estritas (RAISE EXCEPTION) e finaliza com ROLLBACK obrigatório.
-- ============================================================================

BEGIN;

-- 0. VERIFICAÇÃO PRÉVIA DE DEPENDÊNCIAS (public.is_admin)
DO $$
BEGIN
  IF to_regprocedure('public.is_admin()') IS NULL THEN
    RAISE EXCEPTION 'TEST FAILED: Required function public.is_admin() does not exist.';
  END IF;
END;
$$;

-- 1. EXECUTAR A MIGRATION 005 COMPLETA DE FORMA IDÊNTICA AO ARQUIVO DE MIGRATION

-- 1.1 CRIAR TABELA public.checkout_leads
...
-- 1.2 ÍNDICES DE PERFORMANCE E CONSULTA ADMINISTRATIVA
...
-- 1.4 REVOGAR PERMISSÕES PÚBLICAS E CONCEDER APENAS PARA AUTHENTICATED
...
-- 1.5 POLICY EXCLUSIVA PARA ADMINISTRADORES (SELECT, UPDATE, DELETE, INSERT)
...
-- 1.6 RPC PÚBLICA SEGURA: save_checkout_lead
...
-- 1.7 REVOGAR EXECUTE DE PUBLIC E CONCEDER APENAS PARA anon E authenticated
...
-- ============================================================================
-- 2. VALIDAÇÕES TRANSACTIONAIS E ASSERTIVAS DE SEGURANÇA (ESTRITAS COM RAISE EXCEPTION)
-- ============================================================================
...
DECLARE
  ...
  v_policy_count INT;
  v_anon_table_grants INT;
  v_anon_rpc_grants INT;
  v_anon_extra_grants INT;
  v_rpc_res1 JSONB;
  v_rpc_res2 JSONB;
  v_rpc_res3 JSONB;
  v_record_count INT;
...
BEGIN
  -- 2.1 Verificar se a tabela checkout_leads foi criada
  SELECT EXISTS (
    SELECT 1 FROM information_schema.tables WHERE table_schema = 'public' AND table_name = 'checkout_leads'
  ) INTO v_table_exists;
  IF NOT v_table_exists THEN
    RAISE EXCEPTION 'TEST FAILED: Table public.checkout_leads does not exist.';
  END IF;

  -- 2.2 Verificar se RLS está habilitado na tabela
  SELECT rowsecurity FROM pg_tables WHERE schemaname = 'public' AND tablename = 'checkout_leads' INTO v_rls_enabled;
  IF v_rls_enabled IS NOT TRUE THEN
    RAISE EXCEPTION 'TEST FAILED: RLS is not enabled on public.checkout_leads.';
  END IF;

  -- 2.3 Verificar se a Policy Admin foi configurada
  SELECT count(*) FROM pg_policies WHERE schemaname = 'public' AND tablename = 'checkout_leads' AND policyname = 'checkout_leads_admin_all' INTO v_policy_count;
  IF v_policy_count < 1 THEN
    RAISE EXCEPTION 'TEST FAILED: Policy checkout_leads_admin_all is missing on public.checkout_leads.';
  END IF;

  -- 2.4 Verificar que anon NÃO possui acesso direto à tabela checkout_leads
  SELECT count(*) FROM information_schema.role_table_grants WHERE table_schema = 'public' AND table_name = 'checkout_leads' AND grantee = 'anon' INTO v_anon_table_grants;
  IF v_anon_table_grants > 0 THEN
    RAISE EXCEPTION 'TEST FAILED: Role anon still has direct permissions on table public.checkout_leads.';
  END IF;

  -- 2.5 Verificar que anon possui APENAS permissão EXECUTE na RPC save_checkout_lead
  SELECT count(*) FROM information_schema.routine_privileges WHERE routine_schema = 'public' AND routine_name = 'save_checkout_lead' AND grantee = 'anon' AND privilege_type = 'EXECUTE' INTO v_anon_rpc_grants;
  IF v_anon_rpc_grants <> 1 THEN
    RAISE EXCEPTION 'TEST FAILED: Role anon does not have EXECUTE permission on public.save_checkout_lead.';
  END IF;

  -- 2.6 Verificar que anon NÃO possui nenhuma outra permissão extra na RPC
  SELECT count(*) FROM information_schema.routine_privileges WHERE routine_schema = 'public' AND routine_name = 'save_checkout_lead' AND grantee = 'anon' AND privilege_type <> 'EXECUTE' INTO v_anon_extra_grants;
  IF v_anon_extra_grants > 0 THEN
    RAISE EXCEPTION 'TEST FAILED: Role anon has extra non-EXECUTE permissions on public.save_checkout_lead.';
  END IF;

  -- 2.7 Testar criação e execução da RPC save_checkout_lead (SlimSoda e LinfaFlow)
  v_rpc_res1 := public.save_checkout_lead(
    'Jane Doe', 'jane@example.com', '+15559876543', 'US', '+1', 'slimsoda', 'pdp',
    true, 'offer-1', 'SlimSoda PDP', 'https://slimsoda.com/pdp', '/pdp',
    'https://cc.slimsodapowder.com/dtcnew/checkout.php?affid=123', 'visitor-001'
  );
  IF (v_rpc_res1->>'success')::boolean IS NOT TRUE THEN
    RAISE EXCEPTION 'TEST FAILED: RPC execution failed for SlimSoda lead.';
  END IF;

  v_rpc_res2 := public.save_checkout_lead(
    'Alice Smith', 'alice@example.com', '+447911123456', 'GB', '+44', 'linfaflow', 'pdp',
    true, 'offer-2', 'LinfaFlow PDP', 'https://linfaflow.com/pdp', '/pdp',
    'https://cc.linfaflow.com/dtcnew/checkout.php?affid=456', 'visitor-002'
  );
  IF (v_rpc_res2->>'success')::boolean IS NOT TRUE THEN
    RAISE EXCEPTION 'TEST FAILED: RPC execution failed for LinfaFlow lead.';
  END IF;

  -- 2.8 Testar deduplicação de curta janela (60s)
  v_rpc_res3 := public.save_checkout_lead(
    'Jane Doe', 'jane@example.com', '+15559876543', 'US', '+1', 'slimsoda', 'pdp',
    true, 'offer-1', 'SlimSoda PDP', 'https://slimsoda.com/pdp', '/pdp',
    'https://cc.slimsodapowder.com/dtcnew/checkout.php?affid=123', 'visitor-001'
  );
  IF (v_rpc_res3->>'deduplicated')::boolean IS NOT TRUE THEN
    RAISE EXCEPTION 'TEST FAILED: RPC deduplication check failed.';
  END IF;

  -- 2.9 Verificar contagem de registros no estado temporário da transação
  SELECT count(*) FROM public.checkout_leads INTO v_record_count;
  IF v_record_count <> 2 THEN
    RAISE EXCEPTION 'TEST FAILED: Expected 2 inserted records, found %.', v_record_count;
  END IF;

  -- 2.10 Verificar estado da publicação Realtime
  SELECT count(*) FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'checkout_leads' INTO v_realtime_count;
  IF v_realtime_count <> 0 THEN
    RAISE EXCEPTION 'TEST FAILED: Unexpected Realtime publication for public.checkout_leads.';
  END IF;

  -- 2.11 Verificar presença de índices de performance
  SELECT count(*) FROM pg_indexes WHERE schemaname = 'public' AND tablename = 'checkout_leads' INTO v_idx_count;
  IF v_idx_count < 6 THEN
    RAISE EXCEPTION 'TEST FAILED: Expected at least 6 indexes on checkout_leads, found %.', v_idx_count;
  END IF;

  -- 2.12 Verificar constraints de integridade
  SELECT count(*) FROM information_schema.table_constraints WHERE table_schema = 'public' AND table_name = 'checkout_leads' AND constraint_type = 'CHECK' INTO v_constraint_count;
  IF v_constraint_count < 2 THEN
    RAISE EXCEPTION 'TEST FAILED: Expected CHECK constraints on checkout_leads missing.';
  END IF;
...
-- ============================================================================
-- 3. TEARDOWN TRANSACTIONAL (ROLLBACK OBRIGATÓRIO)
-- ============================================================================
ROLLBACK;

-- ============================================================================
-- 4. CONFIRMAÇÃO FINAL APÓS ROLLBACK CONCLUÍDO
-- ============================================================================
