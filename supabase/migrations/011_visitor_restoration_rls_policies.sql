-- ============================================================================
-- ESSENCIAL GOOD - MIGRATION 011: VISITOR CONVERSATION & MESSAGE RESTORATION RLS
-- ============================================================================

BEGIN;

-- 1. POLÍTICA DE LEITURA (SELECT) EM CONVERSATIONS PARA VISITANTE AUTENTICADO
-- Permite que um visitante autenticado (role: authenticated, auth.uid() == visitor_id)
-- consulte exclusivamente suas próprias conversas ativas ou arquivadas.
...
-- Permite que um visitante autenticado (role: authenticated) consulte exclusivamente
-- mensagens de conversas das quais é comprovadamente dono.
...
-- Garante que eventos postgres_changes das tabelas conversations e messages
-- sejam transmitidos com suporte a filtragem RLS.
DO $$
BEGIN
  IF NOT EXISTS (
